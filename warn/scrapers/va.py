...
def scrape(output_dir):
    ...
    output_csv = '{}/virginia_warn_raw.csv'.format(output_dir)
    logger  = logging.getLogger(__name__)

    url = 'https://www.vec.virginia.gov/warn-notices'
    response = requests.get(url)
    logger.info("Page status code is {}".format(response.status_code))
    ...
    # loop over table on each page to get rows
        ...
        logger.info(f"Scraping {len(output_rows)} total rows through page {page}.")
